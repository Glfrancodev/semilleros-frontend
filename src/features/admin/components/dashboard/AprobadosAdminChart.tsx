...
interface AprobacionData {
    ...
    aprobadosPorAdmin: number;
    rechazadosPorAdmin: number;
    pendientesAdmin: number;
    porcentajeAprobados: number;
    porcentajeRechazados: number;
    porcentajePendientes: number;
}

interface AprobadosAdminChartProps {
    filtros?: any;
}

export default function AprobadosAdminChart({ filtros }: AprobadosAdminChartProps) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                console.error('Error fetching aprobados admin chart:', err);
            } finally {
                setLoading(false);
            }
        };
    ...
    const series = [
        data?.aprobadosPorAdmin || 0,
        data?.rechazadosPorAdmin || 0,
        data?.pendientesAdmin || 0,
    ];
        ...
        <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden dark:border-gray-800 dark:bg-white/[0.03] hover:shadow-xl transition-shadow duration-300">
            ...
            <div className="p-6">
                ...
                <div className="flex items-center gap-2 mb-4">
                    <div className="flex-shrink-0 w-5 h-5 text-indigo-500 dark:text-indigo-400">
                        <TaskIcon />
                    </div>
