...
import AdministrativosTableSimple from "./AdministrativosTableSimple";
...
export default function AdministrativosTableWithFilters() {
  ...
  const cargarUsuarios = async () => {
    try {
      ...
      const administrativos = data.usuarios.filter(u => u.Rol.nombre.toLowerCase() === 'administrador');
      setUsuarios(administrativos);
      setTotalUsuarios(administrativos.length);
      setError(null);
    } catch (err) {
  ...
  const handleDelete = async (idUsuario: string) => {
    if (window.confirm('¿Estás seguro de que deseas eliminar este administrativo?')) {
      console.log('Eliminar administrativo:', idUsuario);
  ...
  const handleSubmitUsuario = async (data: AdministrativoFormData) => {
    try {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-gray-600 dark:text-gray-400">Cargando administrativos...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <AdministrativoModal
        ...
        title={selectedUsuario ? 'Editar Administrativo' : 'Crear Administrativo'}
      />
    </div>
  );
}
