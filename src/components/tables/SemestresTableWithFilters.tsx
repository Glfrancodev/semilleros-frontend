...
import SemestresTableSimple from "./SemestresTableSimple";
import CustomSelect from "../common/CustomSelect";
import { Semestre, obtenerSemestres } from "../../services/semestreService";

export default function SemestresTableWithFilters() {
  ...
  const cargarSemestres = async () => {
    try {
      ...
      setSemestres(data.items);
      setTotalSemestres(data.count);
      setError(null);
    } catch (err) {
  ...
  const filteredData = semestres.filter(semestre => {
    const matchesSearch = semestre.numero.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-8">
        <div className="text-gray-600 dark:text-gray-400">Cargando semestres...</div>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-4 rounded-xl border border-gray-200 bg-white p-4 dark:border-white/[0.05] dark:bg-white/[0.03] sm:flex-row sm:items-center">
        <div className="flex-1">
          <input
            type="text"
            placeholder="Buscar por número de semestre..."
            value={searchTerm}
      ...
      {/* Tabla */}
      <SemestresTableSimple
        semestres={paginatedData}
