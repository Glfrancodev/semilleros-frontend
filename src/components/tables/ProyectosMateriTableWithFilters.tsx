...
import ProyectosMateriTableSimple from "./ProyectosMateriTableSimple";
import { Proyecto, obtenerProyectosPorMateria } from "../../services/proyectoService";
import CustomSelect from "../common/CustomSelect";

export default function ProyectosMateriTableWithFilters() {
  ...
  const cargarProyectos = async () => {
    ...
    try {
      setLoading(true);
      const data = await obtenerProyectosPorMateria(idMateria);
      setProyectos(data);
      setError(null);
    } catch (err) {
  ...
  const filteredData = proyectos.filter(proyecto => {
    ...
    const matchesEstado =
      estadoFilter === "all" ||
      (estadoFilter === "pendiente" && (proyecto.estaAprobadoTutor === null || proyecto.estaAprobadoTutor === undefined)) ||
      (estadoFilter === "aprobado" && proyecto.estaAprobadoTutor === true) ||
      (estadoFilter === "rechazado" && proyecto.estaAprobadoTutor === false);
  ...
  const handleRevisar = (idProyecto: string) => {
    navigate(`/docente/proyecto/${idProyecto}`);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Tabla */}
      <ProyectosMateriTableSimple 
        proyectos={paginatedData}
        totalProyectos={filteredData.length}
