...
interface EventosTableProps {
  ...
  onToggleColumn: (column: keyof EventosTableProps['visibleColumns']) => void;
  onAddEvento: () => void;
...
export default function EventosTableSimple({ 
  ...
  const toggleActionsDropdown = (idEvento: string) => {
    setOpenActionsDropdown(openActionsDropdown === idEvento ? null : idEvento);
  };
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-100 p-4 dark:border-white/[0.05]">
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-500 dark:text-gray-400">Total de Eventos:</span>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={visibleColumns.fechaProgramada}
                  onChange={() => onToggleColumn('fechaProgramada')}
                  className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">Fecha Programada</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
        ...
        <Table>
          ...
          <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
            <TableRow>
              ...
              {visibleColumns.fechaProgramada && (
                <TableCell isHeader className="px-5 py-3 font-medium text-gray-500 text-start text-theme-xs dark:text-gray-400">
                  FECHA PROGRAMADA
                </TableCell>
              )}
          ...
          <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
            {eventos.map((evento) => (
              <TableRow key={evento.idEvento}>
                ...
                {visibleColumns.descripcion && (
                  <TableCell className="px-5 py-4 text-start">
                    <span className="text-sm text-gray-600 dark:text-gray-400 break-words max-w-xs">
                      {evento.descripcion || '-'}
                    </span>
                  </TableCell>
                )}
                ...
                {visibleColumns.fechaActualizacion && (
                  <TableCell className="px-5 py-4 text-start">
                    <span className="text-sm text-gray-600 dark:text-gray-400">
                      {formatDate(evento.fechaActualizacion)}
                    </span>
                  </TableCell>
                )}
                ...
                <TableCell className="px-5 py-4 text-start">
                  <div className="relative">
                    ...
                    {openActionsDropdown === evento.idEvento && actionsDropdownPosition && (
                      <>
                        <div
                          className="fixed inset-0 z-10"
