...
interface UsuariosTableProps {
  ...
  visibleColumns: {
    usuario: boolean;      // Foto + Nombre + Apellido
    ci: boolean;
    correo: boolean;
    rol: boolean;
    redesSociales: boolean; // Instagram + LinkedIn + GitHub
    bio: boolean;
    estaActivo: boolean;
  ...
  onToggleColumn: (column: keyof UsuariosTableProps['visibleColumns']) => void;
  onAddUser: () => void;
...
export default function UsuariosTableSimple({ 
  usuarios,
  totalUsuarios,
  visibleColumns,
  showColumnSettings,
  ...
  // Función para obtener las redes sociales disponibles
              ...
              return (
                ...
                {visibleColumns.rol && (
                  <TableCell className="px-5 py-4 text-start">
                    <Badge
                      size="sm"
                      color={usuario.Rol.nombre === "Administrador" ? "primary" : usuario.Rol.nombre === "Docente" ? "warning" : "light"}
                    >
                      {usuario.Rol.nombre}
                    </Badge>
                  </TableCell>
