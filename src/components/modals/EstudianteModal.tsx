...
export default function EstudianteModal({
  ...
  const [errors, setErrors] = useState<Partial<EstudianteFormData>>({});
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors[name as keyof EstudianteFormData]) {
      setErrors(prev => ({ ...prev, [name]: undefined }));
    }
  };

  const validate = (): boolean => {
    const newErrors: Partial<EstudianteFormData> = {};
    ...
    if (!formData.codigoEstudiante.trim()) newErrors.codigoEstudiante = "Código de estudiante es requerido";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setServerError(error.response?.data?.message || error.message || 'Error al guardar estudiante');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            />
            {errors.codigoEstudiante && <p className="mt-1 text-xs text-red-600">{errors.codigoEstudiante}</p>}
          </div>
