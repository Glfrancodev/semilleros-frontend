...
export default function DocenteModal({
  ...
  const [errors, setErrors] = useState<Partial<DocenteFormData>>({});
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (errors[name as keyof DocenteFormData]) {
      setErrors(prev => ({ ...prev, [name]: undefined }));
    }
  };

  const validate = (): boolean => {
    const newErrors: Partial<DocenteFormData> = {};
    ...
    if (!formData.codigoDocente.trim()) newErrors.codigoDocente = "Código de docente es requerido";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setServerError(error.response?.data?.message || error.message || 'Error al guardar docente');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-2xl rounded-xl border border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            />
            {errors.codigoDocente && <p className="mt-1 text-xs text-red-600">{errors.codigoDocente}</p>}
          </div>
